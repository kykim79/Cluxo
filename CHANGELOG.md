...
- **트랙패드 세 손가락 탭으로 라디얼 메뉴 열기** — 트랙패드엔 가운데 버튼이 없어, 세 손가락을 움직임 없이 빠르게 탭하면 라디얼 메뉴가 열린다(`MultitouchService`로 raw 터치 감지, 순수 함수 `isThreeFingerTap`로 분류·unit test). 환경설정 단축키 탭 → 라디얼 메뉴에 토글 추가(기본 ON). 가운데 버튼(마우스)과 독립적으로 동작하며, macOS ‘조회 및 데이터 감지기’ 세 손가락 탭과 함께 발동할 수 있다.

### Changed

- **입력 모니터링(Input Monitoring) 권한 요청 제거** — 키보드 `CGEventTap`이 `.defaultTap`(능동)이라 손쉬운 사용(Accessibility) 권한만으로 동작한다. 입력 모니터링은 redundant라 더 이상 요청·안내하지 않아 권한 부담을 줄였다(손쉬운 사용·화면 녹화 2종만 안내).

## [1.2.9] — 2026-06-24

### Changed

- **라디얼 메뉴 열기 방식 선택 + 기본을 가운데 버튼으로** — 좌클릭 길게 누름은 드래그·텍스트 선택 등 다른 길게-누르기 동작과 충돌이 잦아, 환경설정(단축키 탭 → 라디얼 메뉴)에 ‘마우스로 열기’ 옵션을 추가하고 기본값을 **가운데(휠) 버튼**으로 변경했다. 선택지: `가운데 버튼`(기본) / `좌클릭 길게` / `끄기(단축키만)`. `⌃⌥,` 단축키는 설정과 무관하게 항상 열린다. 가운데 버튼을 트리거로 쓰면 기존 휠 클릭 효과는 표시되지 않는다(트랙패드엔 가운데 버튼이 없어 ‘좌클릭 길게’ 권장).

## [1.2.8] — 2026-06-24

### Added

- **업데이트 자동 알림** — 시작 시 + 하루 1회 백그라운드로 GitHub Releases의 최신 버전을 확인해, 새 버전이 있으면 **메뉴바 아이콘에 빨간 점 배지** + 우클릭 메뉴 맨 위에 `🆕 새 버전 v1.2.x — 지금 업데이트` 항목을 표시한다. 항목을 누르면 환경설정 일반 탭(업데이트 섹션)이 열려 인앱 업데이트로 이어진다. 마지막 확인 시각을 저장해 과도한 요청을 막고, 오프라인 등 실패는 조용히 무시 후 다음 주기에 재시도. 환경설정을 열면 자동으로 한 번 더 확인한다.

## [1.2.7] — 2026-06-24

### Added

- **커서 정지 시 링 페이드 아웃** — idle 타임아웃 후 링이 툭 사라지던 것을 부드럽게 페이드 아웃(`ease.long`, 0.35초)하도록 변경. 다시 움직이면 빠르게(`ease.micro`, 0.12초) 페이드 인하는 비대칭 모션. 즉시 제거(opacity 조건부 렌더)가 아니라 opacity 전환으로 처리한다.

### Fixed

- **휠 클릭·스크롤 후 링이 사라지지 않던 버그** — 클릭·우클릭·휠 클릭·스크롤·흔들기 이벤트가 커서를 다시 보이게만 하고 idle 숨김 타이머를 재시작하지 않아, 그 뒤 마우스를 움직이지 않으면 링이 계속 떠 있던 문제. 이동 외 모든 마우스 활동이 타이머를 재시작하도록 `registerCursorActivity()`로 일원화.
- **가운데·오른쪽 버튼 드래그 시 링이 커서를 따라오지 않던 버그** — CGEventTap 이벤트 마스크에 `otherMouseDragged`·`rightMouseDragged`가 빠져 있어 해당 버튼 드래그 중 위치 갱신이 안 되던 문제. 마스크와 핸들러를 추가해 위치를 추적한다(왼쪽 드래그 전용 시각 효과는 미적용).

## [1.2.6] — 2026-06-07

### Fixed

- **돋보기 + 스크린샷 모드 동시 사용 시 렌즈가 검게 깨지던 버그** — 스크린샷 모드(오버레이 `sharingType=.readOnly`)에서 돋보기 `SCStream`이 자기 오버레이를 재캡처해 렌즈 안이 검게 표시되던 문제. `SCContentFilter`가 자기 앱(Cluxo) 윈도우를 항상 제외하도록 수정해, 스크린샷 모드와 무관하게 깨끗하게 확대된다(데모·녹화 중 돋보기 정상 동작).

## [1.2.5] — 2026-06-02

### Added

- **라디얼 항목 dwell 설명** — 메뉴에서 sector/branch/leaf 위에 커서가 잠시(0.6초) 머무르면 메뉴 하단에 해당 항목의 설명 한 줄이 캡슐로 표시된다. 선택이 바뀌면 사라지고 다시 머물면 재등장. leaf에 설명이 없으면 상위(branch→sector) 설명으로 폴백. 빠른 조작은 방해하지 않고(머물러야만 등장) 입문 학습만 보조. 첫 5회 사용법 안내(help)와 자리를 공유하며 설명 표시 중엔 help를 양보.
...
- **라디얼 메뉴를 ✕로 닫아도 클릭이 막히던 버그** — 중심 ✕(닫기)는 dead zone과 메뉴 바깥 양쪽에서 보이는데, 클릭 닫기는 dead zone(중심)만 처리했다. 메뉴 바깥에서 ✕를 클릭하면 닫히지 않고 좌클릭 소비가 풀리지 않아, ESC를 눌러야만 다시 클릭할 수 있었다. `selectedSector == nil`(선택 없음 = dead zone/바깥 모두) 클릭이면 닫도록 통합.

## [1.2.1] — 2026-06-01

### Added

- **라디얼 메뉴 등장/퇴장 애니메이션** — 한 번에 나타나지 않고 wedge가 12시부터 시계방향으로 순차로 빙 둘러 등장(scale+fade), 닫을 땐 역순으로 사라진다. sub·subSub fan도 sector/branch 선택 시 같은 방식으로 순차 펼쳐짐. 외곽 가이드 원은 wedge가 다 등장한 뒤 마지막에 그려진다(미리 안 그림).
  - 닫기를 2단계로(`radialMenuDismissing`): 입력은 즉시 차단하되 시각은 역순 fade-out 후 제거.
