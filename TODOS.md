...
### 라디얼 메뉴 키보드 내비게이션 (deferred — 디자인 리뷰 2026-06-01)
- **무엇**: ⌃⌥, 로 연 라디얼 메뉴를 키보드로 조작 (화살표로 sector 순환, Tab/숫자로 sub 선택, Enter 실행, ESC 닫기).
- **왜 보류**: 라디얼은 본질적으로 포인팅 제스처(커서 위치/drag 기반 marking menu)라 키보드 nav는 인터랙션 재설계 필요. 무엇보다 라디얼이 노출하는 **모든 설정이 환경설정 창(키보드 완전 접근) + 전역 단축키로 이미 접근 가능** — 기능 차단이 아니라 "빠른 보조 경로"가 마우스 전용일 뿐.
- **언제 검토**: 키보드/스크린리더 사용자 피드백이 실제로 들어오면. 또는 VoiceOver 지원을 진지하게 추진할 때.
- **비용**: 라디얼 선택 상태(`radialMenuSelectedSector/SubItem/SubSubItem`)를 키 입력으로도 갱신하는 경로 추가. RadialMenuView 렌더는 그대로 재사용.
- **의존**: 없음 (독립 작업).
