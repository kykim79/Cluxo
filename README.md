...
> 🎬 **[전체 홍보 영상 보기 (50초, 소리 포함)](docs/Cluxo-promo-ko.mp4)** — 클릭하면 GitHub에서 재생됩니다

## 기능

- **커서 링** — 마우스 주위에 색상 링 표시 (원형/둥근 사각형/둥근 마름모/둥근 육각형, 4단계 크기, 투명도·테두리 두께/스타일, 글로우, 호흡 애니메이션)
- **클릭 효과** — 좌클릭(원형 파동), 우클릭(2중 파동), 더블클릭(버스트), 휠 클릭(회전 호)
- **드래그 인디케이터** — 드래그 시 링이 방향대로 늘어남
- **스크롤 인디케이터** — 스크롤 방향 화살표 (↑↓←→) 표시 + 진폭 비례 크기 (정밀 스크롤과 큰 page 스크롤 한눈에 구분)
- **커서 트레일** — 잔상 효과 (글로우 코멧 테일)
- **돋보기** — 커서 주변 화면 1.5×–4× 실시간 확대. 렌즈 크기 조절, Lanczos 업스케일+샤프닝으로 선명하게, 이동 시 떨림 최소화
- **스포트라이트** — 커서 주변만 밝게 나머지 어둡게. 켜고 끌 때 부드럽게 fade in/out(Mousepose 스타일), 경계 부드러움(feather)·반경 조절
- **키스트로크 표시** — 누른 단축키를 화면 하단에 오버레이. 낯선 외장 모니터(회의실 등) 연결 시 자동 활성화 옵션 (신뢰 모니터는 제외)
- **흔들기 감지** — 마우스를 빠르게 흔들면 커서 위치에 퍼지는 링 표시 ("커서 어디 갔지?" 찾기용). on/off + 민감도 3단계
...
- **라디얼 메뉴 (⌃⌥, 또는 길게 누름)** — 커서 위치에 8 sector 메뉴가 빙 둘러 펼쳐짐. 효과·색·모양·스포트라이트·돋보기·링 외형을 메뉴 유지 상태로 연속 조정 (발표 중 빠른 다중 변경).
  - **2단계 계층** — 항목을 잡고 더 바깥으로 끌면 세부 값이 부채꼴로 펼쳐짐: 스포트라이트(반경·경계), 돋보기(배율·렌즈 크기), 링 외형(크기·투명도·테두리 두께/스타일). 현재 값은 옅은 액센트로 강조.
  - **메뉴 잡아끌기** — 중심을 잡고 드래그하면 메뉴를 원하는 위치로 이동 (모니터 간 이동 포함).
  - **여는 법** — `⌃⌥,` 외에 **마우스 좌클릭 길게 누름(0.5초)** 또는 **트랙패드 길게 누름**으로도 열림 (트랙패드만 쓰는 노트북에 유용). 5pt deadband로 일반 드래그·클릭과 충돌 없음.
...
| `⌃⌥H` | 다음 링 모양으로 순환 (sHape cycle — 원형 → 둥근 사각형 → 둥근 마름모 → 둥근 육각형) |
| `⌃⌥I` | 인스펙터 — 커서 옆 (x, y) 시스템 좌표 표시 |
| `⌃⌥,` | **라디얼 메뉴** — 8 sector 마우스 메뉴. 항목을 잡고 더 바깥으로 끌면 세부 값(반경·배율·크기·투명도·테두리 등)이 2단계로 펼쳐짐. 중심을 잡고 끌면 메뉴 이동. ESC로 닫기. **마우스 좌클릭 길게 누름(0.5초) 또는 트랙패드 길게 누름**으로도 열림 |
